...
channel 有一点类似于 stream(IO)，它就是读写数据的**双向通道**，可以从 channel 将数据读入 buffer，也可以将 buffer 的数据写入 channel，而之前的 stream 要么是输入，要么是输出，channel 比 stream 更为底层

> 数据通过`channel` 对 `buffer` 进行读写
...
3. 从 buffer 读取数据，例如调用 buffer.get()
   - `get()` 到数据后， positon会往后移动一位
   - `get(int index)` 获取指定下标元素就不会
...
![1](../img/Netty.assets/0021.png)
![2](../../img/Netty.assets/0021.png)
![3](../../../img/Netty.assets/0021.png)
![4](../../../../img/Netty.assets/0021.png)
![5](./../img/Netty.assets/0021.png)
...
public class TestByteBufferString {
    public static void main(String[] args) {
        ...
        buffer1.flip(); // 因为buffer1 在 1.位置为写模式，需要换为读模式
...
public static void main(String[] args) throws IOException {
    ...
    // 这里用原子类是因为必须要传引用类型，才不会出现数据不一致(不太清楚)
    // 如果是 int count；匿名内部类无法使用，需要 final int count才能使用
...
Files.walk(Paths.get(source)).forEach(path -> {
    try {
        // 替换是把前缀换为目标目录，否则目标文件\目录名都会是target
...
```java
channel.configureBlocking(false);	// 默认ture,false为非阻塞色模式
...
>   * 客户端发送数据过来，客户端正常（`read==-1`）、异常关闭（`read抛出异常`）时，（**`正常关闭应该是四次挥手，异常RST，但是不知道为啥正常关闭跟异常一样只read1次`**）都会触发 read 事件，另外如果发送的数据大于 buffer 缓冲区，会触发多次读取事件
...
> 事件发生后，要么处理，要么取消（cancel），不能什么都不做，否则下次该事件仍会触发，这是因为 nio 底层使用的是水平触发（添加Key）
...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // SelectionKey sscKey = ssc.register(selector, SelectionKey.OP_ACCEPT); == 上面两句
...
> * 第二次触发了 `sckey` 上的 `read` 事件，但这时 `selectedKeys` 中还有上次的 `ssckey` （**迭代器遍历全部**），在处理时因为没有真正的 `serverSocket` 连上了，就会导致空指针异常
            ...
            while (iter.hasNext()) {
                ...
                if (key.isAcceptable()) {
                    ...
                    if (buffer.hasRemaining()) {
                        // read 1  write 4
                        // 在原有关注事件的基础上，多关注 写事件
    // 如果直接sckey.interestOps(SelectionKey.OP_WRITE)会覆盖
    // 这里可加，因为底层分别是 1，4，8，16这种二进制，因此用 | 会好些
                    ...
                    int write = sc.write(buffer);
                    System.out.println("实际写入字节:" + write);
   // 如果写完需要取消 attach buffer, 才可以释放内存
...
//        EventLoop[] eventLoops = new EventLoop[Runtime.getRuntime().availableProcessors()];
    ...
    static class WorkerEventLoop implements Runnable {
        ...
        public void register(SocketChannel sc) throws IOException {
            ...
            worker.wakeup();
            //注册2：只要 wakeup（）在注册以后即可
            实测老师视频代码 wakeup()在注册之前，如果sleep在wakeup，
            会阻塞在 select()
            /*
            SelectionKey sckey = sc.register(worker, 0, null);
                    sckey.interestOps(SelectionKey.OP_READ);
